...
function parse_commandline()
    ...
    @add_arg_table! s begin
        ...
        "--min_orf", "-m"
            ...
            default = 300
        "--code"
            help = "genetic code from ncbi_trans_table"
            arg_type = Int
            default = 1
        "--strand", "-s"
...
function threeframes(seq::LongDNA{4}, strand::Char, genetic_code::Int)::Vector{Frame}
    frames = Vector{Frame}(undef,3)
    Threads.@threads for frame in 1:3
        start = frame
        rflength = 3*div(length(seq)+1-frame,3)
        finish = start + rflength -1
        frames[frame] = Frame(strand,frame,translate(seq[start:finish], code=ncbi_trans_table[genetic_code]))
    end
    return frames
end

function sixframes(seq::LongDNA{4})::Vector{Frame}
...
function findorfs(frames::Vector{Frame}, genome_length::Int, start_codon::Bool, stop_codon::Bool, min_orf::Int)::Vector{Vector{ORF}}
    ...
    Threads.@threads for (i,frame) in collect(enumerate(frames))
        ...
        while pointer ≠ 0
            ...
            if (pointer - M) ≥ ceil(min_orf/3) && (pointer - M) < 100000 #100k is the limit for hmmsearch
...
function writeorfs(writer::FASTA.Writer, recordid::AbstractString, allorfs::Vector{Vector{ORF}})
    @info recordid
...
function main()
    ...
    instream = endswith(input, ".gz") ? GzipDecompressorStream(open(input)) : open(input)
    reader = FASTA.Reader(instream)
    strand = requested_strand(parsed_args["strand"])
    min_orf = parsed_args["min_orf"]
    genetic_code = parsed_args["code"]
    ...
    for record in reader
        ...
        if strand & FORWARD_STRAND ≠ 0
            frames = threeframes(seq, 'F', genetic_code)
        end
        if strand & REVERSE_STRAND ≠ 0
            rframes = threeframes(reverse_complement(seq), 'R', genetic_code)
